...
import org.bxteam.divinemc.config.DivineConfig; // Purpur - async target finding (Leaf) - config vendored into DivineConfig
...
public class AsyncGoalExecutor {
    ...
    public AsyncGoalExecutor(ServerLevel world) {
        this.world = world;
        this.queue = new SpscIntQueue(DivineConfig.AsyncCategory.asyncTargetFindingQueueSize); // Purpur - async target finding (Leaf)
    }
